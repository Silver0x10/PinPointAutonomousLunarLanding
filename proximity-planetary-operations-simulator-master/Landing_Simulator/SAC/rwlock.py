...
class _RWLockCore(object):
    def __init__(self):
        ...
        self.waiting = Value('i', 0)
        self.owning = multiprocessing.Manager().list()
    ...
    def _acquire_write(self):
        ...
        me = os.getpid()
        if self.state.value == 0 or (self.state.value < 0 and me in self.owning):
            self.state.value -= 1
            self.owning.append(me)
            return True
        if self.state.value > 0 and me in self.owning:
    ...
    def _is_owned(self):
        return self.state.value < 0 and os.getpid() in self.owning

    def _release_save(self):
        # In a exlusively locked state, get the recursion level and free the lock
        with self.cond:
            if os.getpid() not in self.owning:
                raise RuntimeError("cannot release an un-acquired lock")
            r = self.owning
            self.owning = []
            self.state.value = 0
